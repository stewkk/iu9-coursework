...
Целью данной курсовой работы является разработка библиотеки для отслеживания
событий файловой системы, скрывающей детали использования системных вызовов
inotify и предоставляющей удобный интерфейс, не ограничивающий возможности
пользователя.

\section{Обзор предметной области}

TODO: добавить применения отслеживания событий файловой системы. Из примеров:
графический файловый менеджер (проводник), поддержание индекса по файловой
иерархии для быстрого поиска, отслеживание изменений в файлах конфигурации,
автоматический запуск системы сборки при изменениях в файлах (а также lsp),
антивирусы.
...
рекурсивное чтение с одновременным сравнением с предыдущей сохраненной версией.
Этот метод не требует специальной поддержки со стороны операционной системы, но
влечет за собой высокие накладные расходы и высокую вероятность пропустить
некоторые события. Кроме того, некоторые виды событий, таких как переименование
файла, трудно отслеживать таким способом.

Начиная с версии 2.4, в ядре Linux существует механизм под названием
<<dnotify>>, который предоставляет уведомления об операциях над файлами при
помощи сигналов.
...
событий файловой системы под названием <<inotify>>. Этот интерфейс заменил
dnotify, расширив его возможности и гибкость.
...
\begin{itemize}
  ...
  \item Если отслеживается все поддерево каталога и в этом дереве создается
        ...
        сканирование содержимого подкаталога сразу после добавления в список
        отслеживаемых.
\end{itemize}

Основной сценарий использования механизма inotify --- эффективный мониторинг и
кэширования состояния набора объектов файловой системы. Однако, разработчики
пользовательских приложений должны учитывать тот факт, что ошибки в логике
мониторинга или гонки, описанные в ограничениях выше, могут привести к
...
% \subsection{Механизм fanotify}

% \subsection{Язык программирования Go}
